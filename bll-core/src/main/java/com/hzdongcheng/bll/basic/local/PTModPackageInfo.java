...
import com.hzdongcheng.bll.basic.dto.InParamPTModPackageInfo;
...
public class PTModPackageInfo extends AbstractLocalBusiness {
	public String doBusiness(InParamPTModPackageInfo inParam)
	...
	protected String handleBusiness(IRequest request) throws DcdzSystemException {
		InParamPTModPackageInfo inParam = (InParamPTModPackageInfo) request;
		String result = "";

		// 1. 验证输入参数是否有效，如果无效返回-1。
		if (StringUtils.isEmpty(inParam.PackageID)
				|| StringUtils.isEmpty(inParam.CompanyID)
		...
		try {
			...
			setCols1.add("CompanyID", inParam.CompanyID);

			if (StringUtils.isEmpty(inboxPack.OpenBoxKey))
			...
			log.Remark = inParam.RemoteFlag + ",packid=" + inParam.PackageID
					+ ",companyid=" + inParam.CompanyID + ",openboxkey="
					+ inParam.OpenBoxKey;
