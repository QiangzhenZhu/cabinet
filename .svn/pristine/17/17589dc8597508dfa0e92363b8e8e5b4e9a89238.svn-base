...
public class TBBoxDAO{
 private static final Logger log = Logger.getLogger(TBBox.class);
 private static final String TABLE_NAME = "TBBox";
 private static final Class CLASS_TYPE = TBBox.class;

 public TBBoxDAO(){}


	public int insert(SQLiteDatabase database,TBBox obj) throws DcdzSystemException{
	...
	public TBBox find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<TBBox> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		TBBox result = listResult.get(0);
		return result;
	}


	public List<TBBox> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<TBBox> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<TBBox> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
