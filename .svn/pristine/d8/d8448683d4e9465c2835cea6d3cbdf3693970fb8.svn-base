...
import com.hzdongcheng.persistent.dto.OPFunction;
...
public class OPFunctionDAO{
 private static final Logger log = Logger.getLogger(OPFunction.class);
 private static final String TABLE_NAME = "OPFunction";
 private static final Class CLASS_TYPE = OPFunction.class;

 public OPFunctionDAO(){}


	public int insert(SQLiteDatabase database,OPFunction obj) throws DcdzSystemException{
	...
	public OPFunction find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPFunction> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPFunction result = listResult.get(0);
		return result;
	}


	public List<OPFunction> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPFunction> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPFunction> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
