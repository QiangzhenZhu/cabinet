...
public class PAControlParamDAO{
 private static final Logger log = Logger.getLogger(PAControlParam.class);
 private static final String TABLE_NAME = "PAControlParam";
 private static final Class CLASS_TYPE = PAControlParam.class;

 public PAControlParamDAO(){}


	public int insert(SQLiteDatabase database,PAControlParam obj) throws DcdzSystemException{
	...
	public PAControlParam find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PAControlParam> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PAControlParam result = listResult.get(0);
		return result;
	}


	public List<PAControlParam> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PAControlParam> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PAControlParam> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
