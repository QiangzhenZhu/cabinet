...
import com.hzdongcheng.persistent.dto.OPMenu;
...
public class OPMenuDAO{
 private static final Logger log = Logger.getLogger(OPMenu.class);
 private static final String TABLE_NAME = "OPMenu";
 private static final Class CLASS_TYPE = OPMenu.class;

 public OPMenuDAO(){}


	public int insert(SQLiteDatabase database,OPMenu obj) throws DcdzSystemException{
	...
	public OPMenu find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPMenu> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPMenu result = listResult.get(0);
		return result;
	}


	public List<OPMenu> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPMenu> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPMenu> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
