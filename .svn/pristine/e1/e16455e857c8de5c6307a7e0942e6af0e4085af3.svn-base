...
import com.hzdongcheng.bll.basic.dto.InParamTBLockStatusMod;
...
public class TBLockStatusMod extends AbstractLocalBusiness {
	public String doBusiness(InParamTBLockStatusMod inParam) throws DcdzSystemException {
	...
	protected String handleBusiness(IRequest request) throws DcdzSystemException {
		InParamTBLockStatusMod inParam = (InParamTBLockStatusMod) request;
		String result = "";

		// 1. 验证输入参数是否有效，如果无效返回-1。
		if (StringUtils.isEmpty(inParam.BoxNo) || StringUtils.isEmpty(inParam.LockStatus))
		...
		try {
			...
			if (SysDict.BOX_LOCKEDSTATUS_NO == inParam.LockStatus) // 正常
			...
			if (inParam.OperID == "autoLocker") {
				log.Remark = box.BoxNo;
			} else {
