...
public class OPOperatorLogDAO{
 private static final Logger log = Logger.getLogger(OPOperatorLog.class);
 private static final String TABLE_NAME = "OPOperatorLog";
 private static final Class CLASS_TYPE = OPOperatorLog.class;

 public OPOperatorLogDAO(){}


	public int insert(SQLiteDatabase database,OPOperatorLog obj) throws DcdzSystemException{
	...
	public OPOperatorLog find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPOperatorLog> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPOperatorLog result = listResult.get(0);
		return result;
	}


	public List<OPOperatorLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPOperatorLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPOperatorLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
