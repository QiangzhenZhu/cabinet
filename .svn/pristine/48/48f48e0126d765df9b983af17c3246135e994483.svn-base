...
public class PTDeliverHistoryDAO{
 private static final Logger log = Logger.getLogger(PTDeliverHistory.class);
 private static final String TABLE_NAME = "PTDeliverHistory";
 private static final Class CLASS_TYPE = PTDeliverHistory.class;

 public PTDeliverHistoryDAO(){}


	public int insert(SQLiteDatabase database,PTDeliverHistory obj) throws DcdzSystemException{
	...
	public PTDeliverHistory find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PTDeliverHistory> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PTDeliverHistory result = listResult.get(0);
		return result;
	}


	public List<PTDeliverHistory> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PTDeliverHistory> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PTDeliverHistory> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
