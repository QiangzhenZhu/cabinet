...
import com.hzdongcheng.persistent.dto.OPOperRole;
...
public class OPOperRoleDAO{
 private static final Logger log = Logger.getLogger(OPOperRole.class);
 private static final String TABLE_NAME = "OPOperRole";
 private static final Class CLASS_TYPE = OPOperRole.class;

 public OPOperRoleDAO(){}


	public int insert(SQLiteDatabase database,OPOperRole obj) throws DcdzSystemException{
	...
	public OPOperRole find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPOperRole> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPOperRole result = listResult.get(0);
		return result;
	}


	public List<OPOperRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPOperRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPOperRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
