...
import com.hzdongcheng.persistent.dto.PMCompany;
...
public class PMCompanyDAO{
 private static final Logger log = Logger.getLogger(PMCompany.class);
 private static final String TABLE_NAME = "PMCompany";
 private static final Class CLASS_TYPE = PMCompany.class;

 public PMCompanyDAO(){}


	public int insert(SQLiteDatabase database,PMCompany obj) throws DcdzSystemException{
	...
	public PMCompany find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PMCompany> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PMCompany result = listResult.get(0);
		return result;
	}


	public List<PMCompany> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PMCompany> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PMCompany> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
