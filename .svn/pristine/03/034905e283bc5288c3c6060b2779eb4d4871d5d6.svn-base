...
import com.hzdongcheng.persistent.dto.OPRole;
...
public class OPRoleDAO{
 private static final Logger log = Logger.getLogger(OPRole.class);
 private static final String TABLE_NAME = "OPRole";
 private static final Class CLASS_TYPE = OPRole.class;

 public OPRoleDAO(){}


	public int insert(SQLiteDatabase database,OPRole obj) throws DcdzSystemException{
	...
	public OPRole find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPRole> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPRole result = listResult.get(0);
		return result;
	}


	public List<OPRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPRole> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
