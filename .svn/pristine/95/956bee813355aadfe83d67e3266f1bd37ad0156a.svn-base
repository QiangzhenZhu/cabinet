...
public class RMOpenBoxLogDAO{
 private static final Logger log = Logger.getLogger(RMOpenBoxLog.class);
 private static final String TABLE_NAME = "RMOpenBoxLog";
 private static final Class CLASS_TYPE = RMOpenBoxLog.class;

 public RMOpenBoxLogDAO(){}


	public int insert(SQLiteDatabase database,RMOpenBoxLog obj) throws DcdzSystemException{
	...
	public RMOpenBoxLog find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<RMOpenBoxLog> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		RMOpenBoxLog result = listResult.get(0);
		return result;
	}


	public List<RMOpenBoxLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<RMOpenBoxLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<RMOpenBoxLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
