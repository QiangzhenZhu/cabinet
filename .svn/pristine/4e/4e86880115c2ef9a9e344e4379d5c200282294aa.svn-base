...
public class PMPostmanDAO{
 private static final Logger log = Logger.getLogger(PMPostman.class);
 private static final String TABLE_NAME = "PMPostman";
 private static final Class CLASS_TYPE = PMPostman.class;

 public PMPostmanDAO(){}


	public int insert(SQLiteDatabase database,PMPostman obj) throws DcdzSystemException{
	...
	public PMPostman find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PMPostman> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PMPostman result = listResult.get(0);
		return result;
	}


	public List<PMPostman> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PMPostman> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PMPostman> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
