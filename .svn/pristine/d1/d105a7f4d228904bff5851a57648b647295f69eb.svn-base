...
import com.hzdongcheng.persistent.dto.SMAdVideo;
...
public class SMAdVideoDAO{
 private static final Logger log = Logger.getLogger(SMAdVideo.class);
 private static final String TABLE_NAME = "SMAdVideo";
 private static final Class CLASS_TYPE = SMAdVideo.class;

 public SMAdVideoDAO(){}


	public int insert(SQLiteDatabase database,SMAdVideo obj) throws DcdzSystemException{
	...
	public SMAdVideo find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<SMAdVideo> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		SMAdVideo result = listResult.get(0);
		return result;
	}


	public List<SMAdVideo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<SMAdVideo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<SMAdVideo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
