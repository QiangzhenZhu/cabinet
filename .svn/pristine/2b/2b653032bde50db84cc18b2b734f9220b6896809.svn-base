...
import com.hzdongcheng.bll.basic.dto.InParamSMModSignPwd;
...
public class SMModSignPwd extends AbstractLocalBusiness {
	public String doBusiness(InParamSMModSignPwd inParam) throws DcdzSystemException {
		String result;
	...
	protected String handleBusiness(IRequest request) throws DcdzSystemException {
		InParamSMModSignPwd inParam = (InParamSMModSignPwd) request;
		String result = "";

		// 1. 验证输入参数是否有效，如果无效返回-1。
		if (StringUtils.isEmpty(inParam.OperID)
				|| StringUtils.isEmpty(inParam.SystemID)
				|| StringUtils.isEmpty(inParam.InitPasswd))
		...
		setCols0.add("InitPasswd", inParam.InitPasswd);
		try {
