...
import com.hzdongcheng.persistent.dto.PASequence;
...
public class PASequenceDAO{
 private static final Logger log = Logger.getLogger(PASequence.class);
 private static final String TABLE_NAME = "PASequence";
 private static final Class CLASS_TYPE = PASequence.class;

 public PASequenceDAO(){}


	public int insert(SQLiteDatabase database,PASequence obj) throws DcdzSystemException{
	...
	public PASequence find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PASequence> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PASequence result = listResult.get(0);
		return result;
	}


	public List<PASequence> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PASequence> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PASequence> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
