...
import com.hzdongcheng.persistent.dto.SCFtpLog;
...
public class SCFtpLogDAO{
 private static final Logger log = Logger.getLogger(SCFtpLog.class);
 private static final String TABLE_NAME = "SCFtpLog";
 private static final Class CLASS_TYPE = SCFtpLog.class;

 public SCFtpLogDAO(){}


	public int insert(SQLiteDatabase database,SCFtpLog obj) throws DcdzSystemException{
	...
	public SCFtpLog find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<SCFtpLog> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		SCFtpLog result = listResult.get(0);
		return result;
	}


	public List<SCFtpLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<SCFtpLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<SCFtpLog> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
