...
public class OPOperatorDAO{
 private static final Logger log = Logger.getLogger(OPOperator.class);
 private static final String TABLE_NAME = "OPOperator";
 private static final Class CLASS_TYPE = OPOperator.class;

 public OPOperatorDAO(){}


	public int insert(SQLiteDatabase database,OPOperator obj) throws DcdzSystemException{
	...
	public OPOperator find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<OPOperator> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		OPOperator result = listResult.get(0);
		return result;
	}


	public List<OPOperator> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<OPOperator> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<OPOperator> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
