...
public class PMCorpBoxRightDAO{
 private static final Logger log = Logger.getLogger(PMCorpBoxRight.class);
 private static final String TABLE_NAME = "PMCorpBoxRight";
 private static final Class CLASS_TYPE = PMCorpBoxRight.class;

 public PMCorpBoxRightDAO(){}


	public int insert(SQLiteDatabase database,PMCorpBoxRight obj) throws DcdzSystemException{
	...
	public PMCorpBoxRight find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PMCorpBoxRight> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PMCorpBoxRight result = listResult.get(0);
		return result;
	}


	public List<PMCorpBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PMCorpBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PMCorpBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
