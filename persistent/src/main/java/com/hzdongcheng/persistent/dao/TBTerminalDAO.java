...
public class TBTerminalDAO{
 private static final Logger log = Logger.getLogger(TBTerminal.class);
 private static final String TABLE_NAME = "TBTerminal";
 private static final Class CLASS_TYPE = TBTerminal.class;

 public TBTerminalDAO(){}


	public int insert(SQLiteDatabase database,TBTerminal obj) throws DcdzSystemException{
	...
	public TBTerminal find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<TBTerminal> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		TBTerminal result = listResult.get(0);
		return result;
	}


	public List<TBTerminal> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<TBTerminal> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<TBTerminal> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
