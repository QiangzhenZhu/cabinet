...
import com.hzdongcheng.persistent.dto.MBSignInfo;
...
public class MBSignInfoDAO{
 private static final Logger log = Logger.getLogger(MBSignInfo.class);
 private static final String TABLE_NAME = "MBSignInfo";
 private static final Class CLASS_TYPE = MBSignInfo.class;

 public MBSignInfoDAO(){}


	public int insert(SQLiteDatabase database,MBSignInfo obj) throws DcdzSystemException{
	...
	public MBSignInfo find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<MBSignInfo> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		MBSignInfo result = listResult.get(0);
		return result;
	}


	public List<MBSignInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<MBSignInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<MBSignInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
