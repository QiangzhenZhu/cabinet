...
public class PMPostmanBoxRightDAO{
 private static final Logger log = Logger.getLogger(PMPostmanBoxRight.class);
 private static final String TABLE_NAME = "PMPostmanBoxRight";
 private static final Class CLASS_TYPE = PMPostmanBoxRight.class;

 public PMPostmanBoxRightDAO(){}


	public int insert(SQLiteDatabase database,PMPostmanBoxRight obj) throws DcdzSystemException{
	...
	public PMPostmanBoxRight find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<PMPostmanBoxRight> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		PMPostmanBoxRight result = listResult.get(0);
		return result;
	}


	public List<PMPostmanBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<PMPostmanBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<PMPostmanBoxRight> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
