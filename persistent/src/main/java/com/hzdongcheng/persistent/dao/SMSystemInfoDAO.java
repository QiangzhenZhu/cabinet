...
public class SMSystemInfoDAO{
 private static final Logger log = Logger.getLogger(SMSystemInfo.class);
 private static final String TABLE_NAME = "SMSystemInfo";
 private static final Class CLASS_TYPE = SMSystemInfo.class;

 public SMSystemInfoDAO(){}


	public int insert(SQLiteDatabase database,SMSystemInfo obj) throws DcdzSystemException{
	...
	public SMSystemInfo find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<SMSystemInfo> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		SMSystemInfo result = listResult.get(0);
		return result;
	}


	public List<SMSystemInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<SMSystemInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<SMSystemInfo> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
