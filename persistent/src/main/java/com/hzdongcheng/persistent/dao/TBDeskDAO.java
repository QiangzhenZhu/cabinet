...
public class TBDeskDAO{
 private static final Logger log = Logger.getLogger(TBDesk.class);
 private static final String TABLE_NAME = "TBDesk";
 private static final Class CLASS_TYPE = TBDesk.class;

 public TBDeskDAO(){}


	public int insert(SQLiteDatabase database,TBDesk obj) throws DcdzSystemException{
	...
	public TBDesk find(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		List<TBDesk> listResult = DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
		if(listResult == null || listResult.size()==0)
			throw new DcdzSystemException(DBErrorCode.ERR_DB_NORECORD);

		TBDesk result = listResult.get(0);
		return result;
	}


	public List<TBDesk> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE);
	}


	public List<TBDesk> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy) throws DcdzSystemException{
		return DbUtils.executeQuery(database,TABLE_NAME,whereCols,CLASS_TYPE,orderBy);
	}


	public List<TBDesk> executeQuery(SQLiteDatabase database, JDBCFieldArray whereCols,String orderBy,int recordBegin,int recordNum) throws DcdzSystemException{
